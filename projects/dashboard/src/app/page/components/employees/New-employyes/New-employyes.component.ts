...
export class NewEmployyesComponent implements OnInit {
  ...
  OnSubmit() {
    if (this.NewEmployyesForm.valid) {
      if (this.loggedInUser && this.loggedInUser.fullUser) {
        ...
        if (selectedValue === '3') {
          ...
          this.userService.NewUser(this.formData).subscribe(
            (res) => {
              ...
              this.sweetAlertService.success(
                'Success',
                'User Business added successfully.'
              );
              this.router.navigate(['/Users']);
            },
            (err) => {
          ...
          this.userService.NewUser(this.formData).subscribe(
            (res) => {
              console.log('Success', 'User Delivery added successfully.');
              this.sweetAlertService.success(
                'Success',
                'User Business added successfully.'
              );
  ...
  userData(adminId: number, UserCreate: string) {
    ...
    this.formData.append('ConfirmPassword', this._ComfirmPassword.value);
  ...
  passwordsMatchingValidator(fc: AbstractControl): ValidationErrors | null {
  ...
  get _isActive() {
    return this.NewEmployyesForm.controls['isActive'] as FormGroup;
  }
